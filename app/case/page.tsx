...
const caseStudies = [
  {
    id: "rag",
    title:
      "セールスストン：フォーム営業自動化AI【アルファ版登録受付中】",
    industry: "AI",
    challenge: [
...
企業様・法人様のDX化においてコミュニティ運用の知識が不足している課題や、コミュニティ構築（自立分散型組織）へのノウハウが断片的で実装方法がわからないという課題に対して、専門家とのマッチング・対話形式のワークショップで初心者・上級者にとっての限定コンテンツを提供しています。`,
  ...
  {
    id: "DAOathon",
    title: "DAO-A-THON - アジア初のコミュニティ構築実践イベント",
    industry: "DAO構築・コミュニティ",
    challenge: [
      "知識の不足：コミュニティ構築・DAO（自律分散型組織）の概念や実装方法についての理解が不足している",
    ...
    description: `2023年4月1日から9日にかけて、東京でアジア初となるDAO-A-THONが開催されました。1週間かけてコミュニティ構築の概念のアイデア創出から、ツールを用いたコミュニティ構築のデプロイまでを一気通貫で行う本イベントでは、オンライン・オフライン参加累計500名、同時視聴者1000名が集結しました。

キックオフセッションでは、Yosui/Monobundle、Uwaizumi.eth/Unyte、Alex Poon/CharmVerse、Balu/DAOLens、Whiplus/IVS Crypto（敬称略）などの豪華審査員を招き、日本・世界におけるコミュニティ構築の可能性と実現可能性を多方面から探りました。大企業とコミュニティ構築の関係、法整備、市場選定などコミュニティ構築という新領域を日本でどのように広めていくかを模索するセッションが行われました。

4月9日の最終日には日本橋に累計50名が集い、地方創生・音楽・労働環境といったあらゆる領域に対してコミュニティ構築というアプローチが適切かどうかのピッチが行われました。プログラミングの知識がなくても参加可能で、専用の"コミュニティ構築キャンバス"を使用して必要な要素を洗い出し、実際にWeb3上にコミュニティをデプロイする体験を提供しました。`,
    results: [
      "累計500名の参加者が集結し、アジア初の大規模コミュニティ構築DAOイベントを実現",
